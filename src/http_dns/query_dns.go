...
func doQuery(c dns.Client, m dns.Msg, ds, dp string, queryType uint16, close chan struct{}) *dns.Msg {
	...
	default:
		for l := 0; l < 3; l++ {
			// if ds = 10.210.12.10.    change it to be 10.210.12.10    otherwise it wiill lead to ds + : + dp = 10.210.12.10.:53  and it is invalid domain name
			// if ds = ns1.baidu.com. It may work as the domainname is ns1.baidu.com.:53
			...
			if (ee != nil) || (r == nil) || (r.Answer == nil) {
				if r != nil {
					fmt.Println("|TEST|\t", r.Answer)
				}
				fmt.Printf(" doQuery: retry: %s times error: \n", strconv.Itoa(l))
				fmt.Printf("reason: %s\n",  ee.Error())
				if (queryType == dns.TypeA) || (queryType == dns.TypeCNAME) {
					if strings.Contains(ee.Error(), "connection refused") {
						...
						fmt.Printf(" doQuery: response truncated: %v", r)
...
func DoQuery(
	...
	fmt.Println("Fqdn Test| ", domainName)
	fmt.Println("Fqdn Test| ", dns.Fqdn(domainName))

	if queryOpt != nil {
		m.Extra = append(m.Extra, queryOpt)
		fmt.Println("Msg.EXTRA: ", m.Extra)
	}
	var x = make(chan *dns.Msg)
	var closesig = make(chan struct{})

	for _, ds := range domainResolverIP {
		fmt.Println("DoQuery Test| ", ds)
	...
	for _, aa := range a {
		if x, ok := aa.(*dns.A); ok {
			if x.Hdr.Name == dns.Fqdn(d) {
				a_rr = append(a_rr, x)
				fmt.Println(utils.GetDebugLine(), "ParseA:  ", a_rr)
			} else {
